...
먼저 알아둬야할 게 연산자로 연산하는데 숫자가 아닌 falsy, truthy 값들로 있어서는 falsy는 0으로, truthy는 1로 '암묵적 타입 변환'된다.<br />

연산자는 피연산자인 하나 이상의 표현식을 대상으로 산술, 할당, 비교, 논리, 타입, 지수연산 등을 수행해 하나의 값을 만드는 귀여운 놈이다.
...
그 외 쉼표 연산자(`,`), 그룹 연산자(`()`), `typeof` 연산자, 제곱(`Math.pow`)이 가능한 지수 연산자(`**`), 객체 프로퍼티를 삭제하는 `delete`, new, in 등이 있다.<br />
