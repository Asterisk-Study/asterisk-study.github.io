...
### 221104(15일차) 153p ~ 158p
읽기 전용 메모리(ROM)는 읽기만 가능하고 쓸 수 없는 메모리가 아닌 여러 번 읽되 한 번만 쓸 수 있는 메모리를 뜻한다. 가장 초기 형태의 ROM은 비트들을 종이에 구멍을 뚫어 표현한 홀러리스 카드(추후의 IBM 카드)로 스위치와 광감지기를 이용해 카드의 비트를 읽어냈다. 나중엔 여러 장의 카드 대신 길게 이은 종이 테이프에 구멍을 뚫어 사용했다. 이러한 메모리는 일정한 순서로만 읽을 수 있는 순차적 메모리로 장기적으로 저장해야 하는 데이터를 저장할 때 유용하다. 그러나 이러한 데이터를 사용하려면 해당 정보를 RAM으로 읽어 들여야 했는데 보다 나은 저장 기술을 추구한 결과 등장한 것이 마스크(집적 회로 생산 과정에서 사용하는 공판화) 프로그래머블 ROM이다. 하지만 이 마스크의 비용이 상당하기 때문에 비트 패턴이 들어 있는 칩이 대량으로 필요할 경우 프로그래머블 ROM을 만들어 사용했는데 이는 프로그래밍이 가능한 ROM이지만 단 한 번만 프로그램을 작성해 ROM에 넣을 수 있다. 하지만 이 역시 낭비적인 측면이 있어 EPROM(지울 수 있는 PROM, 추후 전기로 내용을 수정할 수 있는 EEPROM도 개발됨)을 만들어 설령 프로그래밍을 잘못 해도 칩 자체를 버리지 않고 특수 처리를 통해 작성한 내용을 지워 다시 쓸 수 있도록 만들었다. 대량 저장장치 중 하나이며 비휘발성인 디스크 드라이브는 다른 유형의 메모리에 비해 상대적으로 느리나 DRAM 등과 달리 데이터 연결을 위한 공간이 필요 없어 기록의 밀도가 매우 높다. 디스크는 블록(섹터) 단위로 주소를 지정해 읽어내며 데이터의 밀도는 안으로 갈수록 더 높아진다. 이러한 디스크 드라이브의 성능을 표현하기 위한 수치가 몇 가지 있는데 탐색 시간(시크타임), 회전 지연 시간이 그것이다. 본래 디스크 드라이브는 하드 드라이브뿐이었으나 후에 플로피 디스크, 자기 테이프, 광학 디스크 등 다양한 디스크 드라이브가 생겨났다.  
<br/>

### 221105(16일차) 159p ~ 163p
플래시 메모리는 가장 최근에 나타난 EEPROM 유형의 매체로 버킷에 전자를 담는 방식으로 작동한다. 플래시 메모리는 RAM처럼 원하는 위치를 마음대로 읽을 수 있지만 이 메모리에 데이터를 기록하기 위해서는 먼저 0을 채워야 한다. 플래시 메모리는 0을 1으로 바꿀 수는 있으나 전체를 지우지 않고 원하는 비트만 0으로 바꾸는 것은 불가능하기 때문이다. 그렇기 때문에 플래시 메모리 메모리는 블록 단위로 나누어지며 전체를 지우더라도 블록 한 단위만 지울 수 있도록 되어 있다. 디스크 드라이브는 점차 고체 상태 드라이브(SSD)로 교체되고 있는데 이 SSD는 디스크 드라이브 모양의 패키지에 넣은 플래시 메모리와 같다. 또한 간혹 메모리에 저장된 비트에 외부 요인(빛, 방사선 등)으로 인해 오류가 발생할 수 있는데 이럴 경우 오류를 감지하는 방식 중 하나가 패리티다. 패리티는 1비트만 데이터가 잘못 돼도 이를 감지하며 데이터에서 1로 설정된 비트의 개수를 세고 그 개수를 비트로 저장하여 짝수인지 홀수인지 저장해 비교하는 방식이다. 하지만 기존 1의 개수도 짝수고 오류도 짝수로 발생했을 경우 이를 감지하지 못하므로 해밍 코드 등과 같은 더 세밀한 오류 감지 시스템(EEC 메모리 칩)을 개발했고 체크섬, 순환 중복 검사 등 값이 변하지 않는 데이터 블록을 검증하는 방식도 생겨났다.   
<br/>
